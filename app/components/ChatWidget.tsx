    ...
    >
      ...
      {!isUser && (
        <div style={{
          ...
          color: "var(--accent)", fontSize: "10px", fontFamily: "var(--font-jetbrains-mono), monospace",
...
export function ChatWidget() {
  ...
  const dict = (dictionaries[language] as any).chat || {
    ...
    welcomeMessage: "Hai! 👋 Saya Asisten Chat portfolio Farhan. Mau tanya apa?",
  ...
  useEffect(() => {
    setMessages([{ id: "welcome", role: "assistant", content: dict.welcomeMessage }]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        ...
        else if (isOpen) { setIsOpen(false); setIsMinimized(false); }
      }
    };
  ...
  const handleSend = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (response.status === 429) {
        setMessages((prev) =>
          prev.map((m) => m.id === asstId ? { ...m, content: "Terlalu banyak pesan. Tunggu sebentar ya. ⏳" } : m)
        );
        return;
      }
      ...
      while (true) {
        ...
        for (const line of chunk.split("\n")) {
          ...
          try {
            ...
            setMessages((prev) =>
              prev.map((m) => m.id === asstId ? { ...m, content: current } : m)
            );
      ...
      setMessages((prev) =>
        prev.map((m) => m.id === asstId ? { ...m, content: dict.errorMessage } : m)
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const closeChat = () => { setIsOpen(false); setIsFullscreen(false); setIsMinimized(false); };
        ...
        {!isOpen && (
          <motion.button
            ...
            style={{
              ...
              border: "none",
              cursor: "pointer",
              boxShadow: "0 4px 20px -2px color-mix(in srgb, var(--accent) 50%, transparent)",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}
          ...
          <>
            ...
            {isFullscreen && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                style={{ position: "fixed", inset: 0, zIndex: 199, backdropFilter: "blur(2px)" }}
              />
            )}
            ...
            >
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  {/* Fullscreen */}
                  <HeaderBtn
                    onClick={() => { setIsFullscreen((v) => !v); setIsMinimized(false); }}
                ...
                <>
                  ...
                  {messages.length <= 2 && !isLoading && (
                    <div style={{
                      ...
                      {[dict.suggestedQ1, dict.suggestedQ2, dict.suggestedQ3].map((q: string, i: number) => (
                        <button
                          key={i}
